...
struct fx_file_def
{
    ...
    s32 pCtrlDef;
    s32 pElementDef;
    s32 MapArrayPlaceholders[2];
};

static_assert( sizeof(fx_file_def) == (13 * sizeof(s32)),
               "FX file definition layout must remain 32-bit" );
...
fx_mgr::fx_mgr( void )
{
    ...
    // Reference registration anchors so every element/controller translation
    // unit is retained when this static library is linked.
    fx_Plane         = 0;
    fx_Sprite        = 0;
    fx_Sphere        = 0;
    fx_Cylinder      = 0;
...
{
    X_PROFILE_SCOPE_CATEGORY( "Context", "fx_mgr::EndOfFrame" );
...
{
    X_PROFILE_SCOPE_CATEGORY( "Context", "fx_mgr::LoadEffect(char*,char*)" );
...
{
    X_PROFILE_SCOPE_CATEGORY( "Context", "fx_mgr::LoadEffect(char*,X_FILE*)" );
    ...
    //
    {
        ...
        // relocated past native-sized arrays which are then rebuilt below.
        {
            ...
            m_pEffectDef[Index]->pDiffuseMap = (texture::handle*)pCursor; pCursor += NBmp * sizeof(texture::handle);

            for( i = 0; i < NBmp; i++ )
                xConstruct( &m_pEffectDef[Index]->pDiffuseMap[i] );
    ...
    //
    {
        ...
        {
            m_pEffectDef[Index]->pDiffuseMap[i].SetName( pNameCursor );
            VERIFY( m_pEffectDef[Index]->pDiffuseMap[i].GetPointer() );
            ...
            // The legacy asset format carries a separate alpha-map name, but
            // PC FX textures already contain their alpha channel. Keep the
            // serialized slot for compatibility without loading a dead resource.
            while( *pNameCursor )
                pNameCursor++;
            pNameCursor++;  // Skip the '\0'.
        }
...
xbool fx_mgr::UnloadEffect( const char* pEffectName )
{
    ...
    for( j = 0; j < m_pEffectDef[i]->NBitmaps; j++ )
        m_pEffectDef[i]->pDiffuseMap[j].Destroy();
...
{
    X_PROFILE_SCOPE_CATEGORY( "Context", "fx_mgr::InitEffect" );
    MEMORY_OWNER( "EFFECT DATA" );
...
{
    X_PROFILE_SCOPE_CATEGORY( "Context", "fx_mgr::AdvanceLogic" );
