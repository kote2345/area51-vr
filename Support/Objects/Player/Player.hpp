...
//      f32                     m_StrainStickSensitivity;             // Stick sensitivity
        ...
        f32                     m_fStrainForwardAccel;                // Gamepad horizontal acceleration
        f32                     m_fStrainYawSensitivity;              // Maximum gamepad yaw rate for this strain
        f32                     m_fStrainPitchSensitivity;            // Maximum gamepad pitch rate for this strain
    };
...
// GetLiveView      -   Gets the raw live player view for viewport setup.
// GetSimulationView-   Gets the authoritative view used by gameplay.
// GetRenderView    -   Gets the current view used by rendering.
...
// GetPlayerViewZone    -   Returns the zone containing the active render view
...
public:
    ...
    static  view&           GetLiveView         ( s32 Player );
            view&           GetSimulationView   ( void );
    const   view&           GetRenderView       ( void ) const;
    ...
    virtual void            UpdateZoneTrack     ( void );
            void            ResetFallStateAfterDiscontinuity( void );
...
public:
    ...
    virtual void            GetProjectileHitLocation (vector3& EndPos, xbool bUseBulletAssist = TRUE);
            ...
            xbool           IsAimToggleEnabled  ( void );
            xbool           IsFiring            ( void );
    virtual void            SpottedByEnemy      ( void ) { m_LastTimeSeenByEnemy = (f32)g_ObjMgr.GetSimulationTimeSeconds(); }
    ...
    virtual xbool           IsPlayer            ( void )                    { return TRUE; }
            ...
            guid            GetCinemaCameraGuid ( void ) const { return m_CinemaSettings.CameraGuid; }
    ...
    virtual xbool           Jump                ( void );
            ...
            void            UpdateLean          ( f32 LeanValue, f32 DeltaTime );
            void            UpdateArmsOffsetForLean( f32 DeltaTime );                         
...
// OnAdvanceSimulation       -   This function gets call ones per game loop. Used to
...
protected:
    ...
    virtual void        OnAdvanceSimulation              ( f32 DeltaTime );
            void        UpdateMultiplayerDeath           ( void );
            void        UpdateWeaponVisibility           ( void );
            void        UpdateJBGLoreUnlock              ( void );
            void        UpdatePlayerView                 ( f32 DeltaTime );
            void        SetCinemaActive                  ( xbool Active );
            PlayerViewContext BuildPlayerViewContext     ( void ) const;
            PlayerViewSample BuildFirstPersonViewSample  ( void );
            PlayerViewSample BuildCinemaViewSample       ( void );
            PlayerViewSample BuildDeathViewSample        ( void ) const;
            void        ApplyViewSample                  ( view& View,
                                                           const PlayerViewSample& Sample );
            void        UpdateFallState                  ( xbool WasFlung );
            void        UpdateCurrentGrenade             ( void );
            void        UpdateGhostLoco                  ( f32 DeltaTime );
            void        UpdateArmsAnimation              ( new_weapon* pWeapon,
                                                           f32 DeltaTime );

    virtual void        OnAnimEvents                ( void );
            xbool       UseFocusObject              ( void );
    virtual void        OnButtonInput               ( f32 DeltaTime );
...
protected:
    ...
    virtual void        UpdateCharacterRotation     ( const f32& DeltaTime );
            ...
            void        EvaluateMovementSpeeds      ( f32 DeltaTime );
            void        ApplyMovementSpeedFactors   ( void );
    virtual void        CalculateStrafeVelocity     ( const vector3& ViewX );
    virtual void        CalculateForwardVelocity    ( const vector3& ViewZ );
            ...
            // Converts separated device input into the canonical look sample.
            void        BuildLookInputSample        ( const PlayerLookInput& Input,
                                                       PlayerLookSample& Sample );
            void        ApplyZoomLookModifiers      ( PlayerLookSample& Sample );
    ...
    virtual void        OnTransformWeapon           ( const matrix4& L2W );
            ...
            // Stunned
            void        ComputeStunnedPitchYawOffset( radian& PitchOffset,
                                                       radian& YawOffset );

            void        StartDeathCamera          ( void );
            void        UpdateDeathCamera         ( f32 DeltaTime );
            void        StopDeathCamera           ( void );
            f32         GetDeathCameraClearDistance( const vector3& Direction,
...
//==============================================================================
    ...
    virtual void            EndFallingToDeath               ( void );
            ...
            void            BeginCinema                     ( void );
            void            UpdateCinema                    ( f32 DeltaTime );
            void            EndCinema                       ( PlayerCinemaFinishReason Reason );
...
public:
    ...
    virtual void                    ScaleVelocity           ( const vector3& PlaneNormal, f32 PerpScale, f32 ParaScale );
            ...
            xbool                   IsCinemaRunning         ( void ) const { return m_CinemaController.IsActive(); }
...
protected:
    ...
    PlayerInput             m_Input;                        // Device-specific input adapter
    PlayerLook              m_Look;                         // Look-input math: accel ramp, stick sensitivity
    PlayerLookSample        m_LookSample;                   // Current look input sample
    PlayerMovement          m_Movement;                     // Device-aware movement response
    PlayerMoveInput         m_MoveInput;                    // Current movement input
    PlayerCombat            m_Combat;                       // Combat-intent and aim-assist policy
    PlayerAnimationDispatcher<animation_state>
                            m_AnimationDispatcher;           // Typed transition policy
    PlayerInventory         m_InventoryInteraction;          // Weapon/use intent policy
    PlayerStatus            m_Status;                        // Per-player lifecycle and status timers
    PlayerView              m_ViewController;                // Simulation/presentation view policy
    PlayerViewSequence      m_LockedView;                    // Trigger-driven first-person view sequence
    PlayerDeathCamera       m_DeathCamera;                   // Player-owned death presentation camera
    PlayerViewSample        m_SimulationViewSample;          // Authoritative view selected once per simulation step
    xbool                   m_SimulationViewInitialized;
    zone_mgr::tracker       m_RenderViewZoneTracker;         // Tracks only the final rendered view position
    guid                    m_RenderViewZoneSourceGuid;
    xbool                   m_RenderViewZoneInitialized;
    xbool                   m_RenderViewZoneUsesCameraSeed;
    xbool                   m_RebaseRenderViewZone;
    xbool                   m_bApplyingTeleport;
    ...
    s32                     m_LocalSlot;
    f32                     m_MaxFowardVelocity;            // Maximum horizontal movement velocity
    f32                     m_JumpVelocity;                 // How much velocity he has at the take off of the jump

    vector3                 m_ForwardVelocity;              // Forward velocity in centimeters per second.
    vector3                 m_StrafeVelocity;               // Strafe velocity in centimeters per second.
    f32                     m_fForwardAccel;                // Scalar rate of horizontal movement acceleration
...
protected:
    ...
    // Combined gameplay movement intent for the current frame.
    f32                     m_fMoveValue;
    f32                     m_fStrafeValue;
...
protected:
    ...
    PlayerCinemaSettings        m_CinemaSettings;
    PlayerCinema                m_CinemaController;
    PlayerViewSample            m_CinemaViewSample;
    xbool                       m_CinemaPhysicsCaptured;
    xbool                       m_CinemaSolveActorCollisions;
    xbool                       m_CinemaLocoCollision;
    xbool                       m_CinemaLocoGravity;
...
protected:
    ...
    rhandle<texture>        m_MissionFailedBmp;
    ...
    struct manned_turret
    {
        ...
        zone_mgr::zone_id   PreviousZone1;
        zone_mgr::zone_id   PreviousZone2;
        matrix4             AnchorL2W;
    };
...
{
    m_Look.ScaleStickSensitivity( rMultiplier );
}
